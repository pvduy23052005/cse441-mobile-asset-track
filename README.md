...
| Tác nhân / Phân hệ | Thành viên Phụ trách | Phạm vi Chức năng Cốt lõi |
| :--- | :--- | :--- |
| **1. Kiến trúc Backend & Frontend, Operator & Master Data** | **Phùng Văn Duy (2351170589)** | • **Kiến trúc Hệ thống Backend & Frontend:** Thiết kế kiến trúc tổng thể hệ thống Frontend (Flutter) & Backend (BaaS/Cloud), xây dựng cơ chế Offline cho Operator (SQLite Queue & Auto Sync — NFR-06).<br>• **Operator Workflow:** Quét mã QR (`mobile_scanner`), Hộ chiếu thiết bị, khai báo số giờ chạy/km, tạo Ticket SOS khẩn cấp + chụp ảnh camera, hủy ticket pending.<br>• **Quản lý Máy móc & Nhân sự:** CRUD máy móc, tạo & in tem mã QR, cấu hình mốc bảo trì (`pm_threshold_hours`), quản lý cẩm nang lỗi nhanh, quản lý tài khoản & import nhân sự Excel.<br>• **Cloudflare Image Upload:** Tích hợp upload & tối ưu hóa ảnh hiện trường lên Cloudflare (Cloudflare R2 / Images). |
| **2. ME Workflow, Offline Engineer, Dashboard Supervisor & Nghiệm thu** | **Lê Quý Dương (2351170587)** | • **Chế độ Offline cho Engineer (NFR-06):** Cache danh sách PM Checklist & Ticket, lưu kết quả checklist/ảnh minh chứng/vật tư khi mất mạng, tự động đồng bộ khi có kết nối.<br>• **ME Engineer Workflow:** Nhận Push Notification FCM khẩn cấp, tiếp nhận Ticket sửa chữa, thực hiện PM Checklist bảo trì kèm ảnh minh chứng, ghi log vật tư tủ nhanh & gửi đề xuất linh kiện đắt tiền.<br>• **Dashboard Supervisor:** Giám sát thời gian dừng máy (Downtime) & chỉ số hiệu suất OEE phân xưởng qua Real-time Dashboard (`fl_chart` - Pie Chart & Bar Chart), cảnh báo sự cố thiết bị.<br>• **Nhận Nghiệm thu từ Engineer:** Tiếp nhận phiếu nghiệm thu từ Kỹ sư ME, kiểm tra ảnh minh chứng đối chứng & linh kiện thay thế, ký tên điện tử nghiệm thu (`signature`) trực tiếp trên màn hình cảm ứng để đưa máy về `Active` hoặc từ chối kèm lý do (`Rejected`).<br>• **Phê duyệt Vật tư:** Phê duyệt đề xuất linh kiện đắt tiền vượt ngưỡng chi phí cấu hình của phân xưởng. |
