...
| :--- | :--- | :--- |
| **1. Operator, ME Workflow, Quản lý Máy móc & Người dùng** | **Phùng Văn Duy (2351170589)** | • **Operator & ME Workflow:** Quét mã QR (`mobile_scanner`), Hộ chiếu thiết bị, khai báo số giờ chạy/km, tạo Ticket SOS khẩn cấp + chụp ảnh camera, hủy ticket pending, tiếp nhận Ticket sửa chữa, thực hiện PM Checklist kèm ảnh minh chứng, ghi log vật tư tủ nhanh & đề xuất linh kiện, SQLite offline queue (NFR-06).<br>• **Quản lý Máy móc:** CRUD máy móc, tạo & in tem mã QR, cấu hình mốc giờ bảo dưỡng (`pm_threshold_hours`), quản lý cẩm nang lỗi nhanh.<br>• **Quản lý Người dùng & Nhân sự:** Quản lý tài khoản, phân quyền vai trò (`operator`, `me_engineer`, `supervisor`), import nhân sự từ file Excel.<br>• **Cloudflare Image Upload:** Tích hợp upload & tối ưu hóa ảnh hiện trường lên Cloudflare (Cloudflare R2 / Images). |
| **2. Dashboard Supervisor & Nhận Nghiệm thu** | **Lê Quý Dương (2351170587)** | • **Dashboard Supervisor:** Giám sát thời gian dừng máy (Downtime) & chỉ số hiệu suất OEE phân xưởng qua Real-time Dashboard (`fl_chart` - Pie Chart & Bar Chart), cảnh báo sự cố thiết bị.<br>• **Nhận Nghiệm thu từ Engineer:** Tiếp nhận phiếu nghiệm thu từ Kỹ sư ME, kiểm tra ảnh minh chứng đối chứng & linh kiện thay thế, ký tên điện tử nghiệm thu (`signature`) trực tiếp trên màn hình cảm ứng để đưa máy về `Active` hoặc từ chối kèm lý do (`Rejected`).<br>• **Phê duyệt Vật tư:** Phê duyệt đề xuất linh kiện đắt tiền vượt ngưỡng chi phí cấu hình của phân xưởng. |
